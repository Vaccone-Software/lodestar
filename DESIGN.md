...
**Normal mode is vim.** `Vim` (LodestarCore) is a pure state machine over the buffer: motions, operators, counts, text objects, find, replace, join, case, undo, redo, dot repeat, and the visual modes, tested without an app. Stock vim first; this user's own extensions come after, additively, on stock's weak spots. Two deliberate deviations, decided before the first line: `⏎` commits in every mode (`⇧⏎` is the newline; a NeoVim hand already uses enter for jumps), and only `y` and `p` touch the system clipboard, so `dd` then `p` pastes what was yanked and the strip holds yanks and drafts and nothing else. `esc` clears a pending command before it means anything else, leaves visual mode before that, and only then closes.
