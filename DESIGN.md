...
**One cursor, two inputs.** Typed characters land at the cursor; the recognizer's settled words land there too, joined by a spacing rule so a typed identifier and a spoken sentence read as prose. The recognizer's volatile words draw dimmed after the cursor as a ghost, so anything typed while a ghost stands lands before what is still being said. Backspace takes the grain of the last input: a character after typing, a word after speech, because a transcript's unit of error is the word and a keyboard's is the character. The ghost is never touched; it settles on its own.
...
**Normal mode is vim.** `Vim` (LodestarCore) is a pure state machine over the buffer: motions, operators, counts, text objects, find, replace, join, case, undo, redo, dot repeat, and the visual modes, tested without an app. Stock vim first; this user's own extensions come after, additively, on stock's weak spots. Two deliberate deviations, decided before the first line: `⏎` commits in every mode (`⇧⏎` is the newline; a NeoVim hand already uses enter for jumps), and only `y` and `p` touch the system clipboard, so `dd` then `p` pastes what was yanked and the strip holds yanks and drafts and nothing else. Find motions are document-wide without wrapping: `f`, `F`, `t`, and `T` cross line breaks, and the moment one is pending the first instance of each reachable character lights in the macOS accent color — the letters themselves recolored and weighted, never a wash behind them, because vibrancy composites a background wash on this glass into nothing. The named match stays lit for `;` and `,` until the next navigation or edit. `esc` clears a pending command before it means anything else, leaves visual mode before that, and only then closes.
