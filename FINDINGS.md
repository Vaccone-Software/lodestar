...
## 8. Chromium never reports window death — liveness must be asked, not awaited (2026-08-10)

The `lode b d` blank-window report. The model's only death signal was `kAXUIElementDestroyed`, and Chromium never delivers it: in a 3.5-hour session, 41 destroy notifications arrived (Finder, System Settings, Zoom, Console) and zero from Brave across dozens of window closes — 30 Brave records the model called alive were gone from the window server. A ghost titled `New Tab - Brave - Default` matched the profile summon, won `candidates.first` (dictionary order — the "inconsistently"), became the sole layout member (parking everything else), and every AX call on its dead element silently no-opped: Brave activated with nothing placed.

A controlled experiment (open a Brave window, cache its element, close it via its AX close button, interrogate the corpse) pinned the behavior:

- `AXObserverAddNotification(…, kAXUIElementDestroyed)` on the window element returns **success** — registration works; the notification simply never fires (waited 5s). There is nothing to await and nothing to log.
- The dead element answers `kAXRoleAttribute` with **`.invalidUIElement` (-25202)** — exact, and distinct from a hung app's `.cannotComplete` (-25204), which must keep reading as alive or a beachball becomes mass window-death. (`AX.copy` collapses all errors to nil, so verification reads the raw `AXError`.)
- The window-server all-list dropped the closed window within 3s, while a **minimized** window stays listed and stays `.success` — and tab-hidden/parked/other-Space windows stay listed too (§3), so list-absence is a sound death signal. Presence proves nothing: dead windows can linger server-side app-dependently (§3's Ghostty lingerers), so the list never resurrects.

The fix, two tiers funneling into one `bury` path (which fires `onDestroyed`, so layouts retile and parking forgets, same as a real destroy): `sweepAgainstWindowServer()` — one hang-immune CG call burying every alive record the server no longer lists — runs at every destination lookup, focus sync, `lode 0`, and breath save; `verify(id)` — the per-element role read where only `.invalidUIElement` kills — gates every verb that acts on a specific window (summons, marks, breaths, index jumps, history walks, `place` itself). Ghost accumulation is what made per-lookup verification cheap to keep cheap: the sweep buries the day's dead in one call, so element probes touch only windows the server still lists. Destination ties are no longer dictionary order: most recently focused wins, then newest id (§4 monotonicity).

## 9. Concurrent update applies destroy the install — one flight, ever (2026-08-13)

The first field auto-update: three presses of Check for Updates within 90 seconds spawned three full check→download→stage→apply pipelines in the same process (the old instance keeps running until a successor takes the pid file, so "apply started" never meant "done"). Each apply blindly cleared `lodestar.app.previous` before swapping — apply 2 deleted apply 1's rollback copy, apply 3 deleted the last surviving bundle and then found the install slot empty. The watchdogs compounded it: rollback was `rm -rf $APP; mv $PREVIOUS $APP` unconditionally, so a second watchdog could delete the app when `.previous` no longer existed to restore. End state: no bundle on disk, no process running.

- The check flag guarded only the network window; a staged/applying run offered no re-entry protection at all.
- The repeat presses were rational: the force path flashed once at apply and went silent, so "nothing happened" invited another press.
- Fixes, in the order they matter: a phase machine (`idle → checking → ready → applying`, applying terminal for the process — a repeated press joins the run and says so in a flash); swap preflight (install must exist, `.previous` must be absent — a lingering `.previous` means an unresolved story, stand down, never delete it); watchdog rollback gated on `[ -d "$PREVIOUS" ]` — deleting the app with nothing to put back is never the answer.
- Recovery lesson: the release pipeline's `dist/` holds the exact notarized bundle — reinstall is one `ditto -x -k` away.
