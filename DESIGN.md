...
**Music steps aside.** Opening the microphone on a Bluetooth headset drops the whole device to its hands-free profile — mono, telephone-band, a separately remembered volume — because the classic radio cannot carry music out and voice in at once. No app can prevent it and every app that opens the mic causes it, so while the draft listens there is nothing worth hearing: whatever is playing is paused, and resumed once the output is a music profile again. Derived, never configured — it acts only when the input about to be read and the current output are the same Bluetooth radio, only when something held through two roll calls of the system's outputters (a keyboard-click app flickers in one; music stands in both), and it resumes only what it watched stop. The pause key is a toggle aimed at whichever app owns Now Playing, so it is distrusted at every step: sent, then checked, and the one wrong action it could take — starting something silent — is watched for and undone on the spot. Any doubt collapses to doing nothing, which is why there is no setting: the condition is the knob, and a hand that wants the headset mic with music left running can pin a different input and the gate never opens.
