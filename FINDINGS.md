...
The `hyper b d` blank-window report. The model's only death signal was `kAXUIElementDestroyed`, and Chromium never delivers it: in a 3.5-hour session, 41 destroy notifications arrived (Finder, System Settings, Zoom, Console) and zero from Brave across dozens of window closes — 30 Brave records the model called alive were gone from the window server. A ghost titled `New Tab - Brave - Default` matched the profile summon, won `candidates.first` (dictionary order — the "inconsistently"), became the sole layout member (parking everything else), and every AX call on its dead element silently no-opped: Brave activated with nothing placed.
