...
- **`E` on an image opens it across the display**, fitted whole, the strip gone beneath it. Pinch to zoom and pan with two fingers, as with any image; nothing is clicked. `esc` or `⏎` steps back to the strip; `S` goes on to save it.
