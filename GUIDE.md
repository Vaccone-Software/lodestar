...
- **`E` on an image opens it large**, above the strip with the card lit beneath, as big as the display allows without going past its own pixels. `esc` or `⏎` steps back to the strip; `S` goes on to save it.
- **`S` on an image saves it under a name you type.** The band offers a name made from where the image came from and when, like a screenshot's, so `⏎` alone is enough. Type to name it instead: a slash inside the name is a subfolder, a name starting with `/` or `~` is a path of its own, a trailing slash is a folder that takes the offered name, and `.jpg` or `.tiff` writes that format instead of PNG. Everything else lands in `clipboard.save-to`, which is Downloads unless you say otherwise. `esc` steps back with nothing written.
